...
function cekhisri(kelpas){
  ...
  if(kelpas==''){
    var pilpas = 'rs';
    var jenpas = td1 + td2 + pilpas;
  } else {
    var pilpas = kelpas;
    var jenpas = td1 + td2 + kelpas;
  }
  $.ajax({
      type: 'POST',
      url: '<?php echo base_url(); ?>markas/core1/chisri/' + jenpas,
      success: function(data) {
        $('#chisaduh').addClass('hidden');
        if(data!='proses'){
          prohisri(jenpas);
        }
        cekhisrj(jenpas);
  ...
  fillgrid(pilpas,pegsu);
}

function prohisri(kelpas){
  if(kelpas==''){
    var jenpas = 'rs';
  } else {
    var jenpas = kelpas;
  ...
  $.ajax({
      type: 'POST',
      url: '<?php echo base_url(); ?>markas/core1/phisri/' + jenpas,
      success: function(data) {
        $('#chisaduh').addClass('hidden');
      }
  });
...
/*  if(kelpas==''){
  ...
  $.ajax({
      type: 'POST',
      url: '<?php echo base_url(); ?>markas/core1/chisrj/' + jenpas,
      success: function(data) {
        $('#chisaduh').addClass('hidden');
        if(data!='proses'){
          prohisrj(jenpas);
        }
        fillgrid(jenpas,pegsu);
      }
  });
}

function prohisrj(kelpas){
  var jenpas = kelpas;
  $.ajax({
      type: 'POST',
      url: '<?php echo base_url(); ?>markas/core1/phisrj/' + jenpas,
      success: function(data) {
      }
  });
}
...
function fillgrid(kelpas,supeg){
  if(kelpas==''){
    var jenpas = 'rs';
  } else {
    var jenpas = kelpas;
  ...
  var f2 = toDate($('#fl_tgl2').val());
      ...
      var td1 = f1.toISOString().split('T')[0];
      var td2 = f2.toISOString().split('T')[0];
      var url = "<?php echo base_url(); ?>markas/core1/fillqbill/"+jenpas;
      var tglctk = new Date();
